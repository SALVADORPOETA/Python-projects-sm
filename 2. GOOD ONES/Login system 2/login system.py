...
def login():
	...
	if user == "Daniel" and code == "1234":
		...
		def Total():
			...
			string_bill = str("Rs. %.2f" %totalcost)
		...
		#Bill = Label(f2, text="Bill", font=("calibri", 20))
		...
		# buttons
		btn_reset=Button(f1, bd=5, fg="black", bg="lightblue", font=("arial", 16, "bold"), width=10, text="Reset", command=Reset, cursor="hand2")
		btn_reset.grid(row=8, column=0)

		btn_total = Button(f1, bd=5, fg="black", bg="lightblue", font=("arial", 16, "bold"), width=10, text="Total", command=Total, cursor="hand2")
	...
	# all alerts, when someone try to enter wrong username and password
		...
		messagebox.showerror("Invalid", "The Password fiel is required")

	elif user != "Daniel" and code != "1234":
...
def main_screen():
	...
	screen = Tk()
	screen.geometry("1080x720+150+10")
	...
	Button(mainframe, text="Login", height="2", width=23, bg="#ed3833", fg="white", bd=0, cursor="hand2", command=login).place(x=100, y=250)
	Button(mainframe, text="Reset", height="2", width=23, bg="#1089ff", fg="white", bd=0, cursor="hand2", command=reset).place(x=300, y=250)
	Button(mainframe, text="Exit", height="2", width=23, bg="#00bd56", fg="white", bd=0, cursor="hand2", command=screen.destroy).place(x=500, y=250)
